...
function SetPanelStateByValue(dpId, value, easingOut, directionOut, durationOut, easingIn, directionIn, durationIn) {
    var $state = $('#' + dpId).children('[data-label="' + value + '"]');
    if ($state.length != 0) {
        var stateid = $state.first().attr('id')
        SetPanelState(dpId, stateid, easingOut, directionOut, durationOut, easingIn, directionIn, durationIn);
    } else if (isNaN(value)) {
        return;
    } else {
        var stateid = 'pd' + value + dpId;
...
function DragWidget(event) {
    ...
    if (bIE) {
        ...
        if (event.changedTouches) {
            x = event.changedTouches[0].pageX;
            y = event.changedTouches[0].pageY;
            if (widgetIdToDragFunction[widgetDragInfo.widgetId]) event.preventDefault();
        } else {
            x = event.pageX;
            y = event.pageY;
        }
...
function SuppressClickAfterDrag(event) {
    if (bIE) { document.detachEvent("onclick", SuppressClickAfterDrag); }
...
function StopDragWidget() {
    ...
    if (widgetDragInfo.hasStarted) {
        ...
        if(bIE) { document.attachEvent("onclick",SuppressClickAfterDrag); }
